---
title: "Application of Garman & Klass measures"
author: "Martina Chiesa and Matteo Zucca"
output:
...
```{r}
rm(list = ls())
suppressWarnings(suppressMessages(library(rugarch))) #fits Arima and Garch models
suppressWarnings(suppressMessages(library(forecast)))  #Rob Hyndman's package, contains Acf(.) function (it excludes first lag, acf(.) doesn't)
suppressWarnings(suppressMessages(library(xts)))       #xts object has some additional features rather than ts object (R's default)
suppressWarnings(suppressMessages(library(quantmod)))  #with getSymbols function we can download data
...
### 1.2) Download and store Data 
 ...
 Notice: section 1.2 should be executed only first time to download data, otherwise results will change day by day.
...
RawDataFinal = data.frame(RawData, 
                          log.return = c(NA, diff(log(RawData$Adjusted))), #log-returns
                          check.names = TRUE)
```

 Note: we have to include a NA in order to match the dimension of dataframe and requirements set by garmanklass.measure function
...
```
 ...
 Obs: adjusted prices are a transformation of close price that takes into account splits and dividends of the firms


## 2) Preliminary Analysis of Original Data 
 
 Purpose of this section is to prove that, when dealing with stock prices, modelling the mean isn't possible since homoeskedasticity assumption doesn't hold
...
```
  ...
  * Serie of closing price isn't stationary in mean: we have clusters of variability (stylized fact of financial ts)
...
```
 ...
 Comment: decay of ACF is too slow -> serie is clearly non-stationary tests for Unit Root will lead to this conclusion (omitted)

 Final Comments: 
 
 * Both series of close price and its log transform are non stationary and heteroskedastic
 * Log scale allows to appreciate better the price changes. However it isn't a variance-stabilizing transformation.
 * In next session, we'll use difference of logs to get a stationary (in mean) serie. In fact, this correspond to log-return
...
```
  ...
  * Daily log-returns move around a mean close to zero (similarly to a WN); 
  * There are periods with different variability around the mean (heteroskedasticity)
      
    This is a consequence of clusters of variability (stylized facts of financial TS)
  * We have several outliers depending on exogenous shocks (e.g. economic crisis)
  * From ACF, we can notice some lags that exceed the significance bands.
    However, in absolute value correlations aren't that high.
  * Normality is rejected: Distribution seems leptokurtic.
 ...
 It can be considered a (very) proxy measure for conditional variance, then if there's serial autocorrelation it can not be a WN.
...
```
 ...
 Comments:
   ...
   * Changing order of Arma and distribution of error doesn't improve the problem of correlation of residuals.
   * Residuals of an Arma model aren't WN.
   * Model for the mean isn't adeguate.
 ...
 **Diagnostic on Standardized Residuals**
...
```
 ...
 Comments: 
   ...
   * Simple-Garch may not be the best model but it has satisfying results (residuals NWN)
   * Diagnostic on residuals doesn't show any several problems. However we can notice the influence of outliers. This behaviour is common and well-studied in finance, e.g. the theory of Black Swan.


### 5.2) GJR-GARCH

 We introduce a leverage parameter due to the stylized fact that conditional volatility is higher when returns are negative.
...
```
 ...
 Comments:
   ...
   * GJR-Garch has slightly better diagnostic than s-Garch
   * Does it worths an higher complexity (leverage parameter)?
      Answer has to be based on error measures.


### 5.3) Further developments

 Literature proposed decades of garch models starting from 80's. For example: T-Garch, E-Garch, i-Garch and the entire family of f-Garch. They differ by several aspects.
 
 However often the simple-Garch(1,1) has satisfying results and increasing complexity doesn't worth it.


## 6) Forecasting (Ex-Post) 

 Notice:
  * We assume Garman-Klass measures as a less noisy proxy than squared return for volatility at time t.
  * We don't perform any kind of external validation. As a consequence, the error may be underestimated.
...
```
 ...
 Comments:
   ...
   * Both models, compared to naive, performs well on predicting the volatility.
   * GJR-Garch allows a reduction of 7 percentage point w.r.t. S-GARCH in Scaled RMSE.
   * S-Garch doesn't works well on predicting the conditional variance if compared to naive model. However with the leverage parameter we can improve this inefficiency.
